...
VS
{
    ...
    PixelInput MainVs( VertexInput i )
    {
        ...
        // Take XY components from vertex world-space position and adjust scale by user-defined variable.
        // We're dividing these coords because by default they're very small, this will make UV "larger" 
        // and make noise values appear more natural.
        float2 vPosWs = o.vPositionWs.xy / (g_flNoiseScale * 50);
        // Generate Simplex2D nosie using adjusted XY components of world-space vertex position, add g_flTime constant
        // to make noise "moving" forward for better sway effect. Multiply this noise to increase intensity of vertex displacement. 
        float flNoise = Simplex2D( vPosWs + (g_flTime * g_flSwaySpeed) ) * g_flDisplacementStrength;
        // Add the result noise value to world-space vertex coordinates on XY axis.
        // Multiply it by red vertex color channel so this noise value will be 0 in areas where red color isn't painted.  
        o.vPositionWs.xy += flNoise * o.vVertexColor.r;
        // Last thing is converting world-space coords to projection space. While we have updated vPositionWs, 
        // vPositionPs still uses outdated coordinates, generated in ProcessVertex!
